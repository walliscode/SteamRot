...
collision checks that work at scale 1.0 but break the moment the camera zooms in
or pans. This document explains the two spaces, the transform that connects
...
Screen space (also called _window space_ or _pixel space_) is the 2D coordinate
...
**Key insight**: screen space is _fixed_ — zooming or panning the camera never
...
A _camera_ (in SFML: an `sf::View`) defines the rectangle of world space that is
currently projected onto the screen. Conceptually it answers: "which world
...
This means a pixel (px, py) at zoom 1.0 represents a _different_ world position
...
Both functions require the _same_ `sf::View` that is active during rendering —
...
- **Single point of failure**: if the conversion is wrong it is wrong in one
  place
...
position must be mapped through that _same zoomed world view_ to get a correct
...
Sometimes a _canvas pixel_ (a fixed position on screen like the centre of a
