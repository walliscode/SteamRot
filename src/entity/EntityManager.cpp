...
{
	for (const size_t& i : m_entitiesToRemove) //for all items in the to add list ...
	{
		std::get<0>(*(*m_pool).getData())[i] = false; //set the active vector to false at the passed entity index
		auto deadEntity = std::find(m_entities.begin(), m_entities.end(), i);
