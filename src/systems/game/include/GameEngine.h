...
public:
  ...
  void run(size_t numLoops = 0); // Run the game, numLoops is used specifically
                                 // for the simulation function
  void update(); // calls the SceneManager update function. This may be
                 // uncessary separation but will leave for no
  ...
  json toJSON(); // convert parts of the GameEngine object to JSON, take a
                 // string as a container name for the json
  void createJSON(
      const std::string &directoryName,
      const std::string
          &fileName); // Create a JSON file from parts specificed by toJSON
                      // functions from each class. This enables multiple jsons
                      // to be created with different structures
  json extractJSON(const std::string &directoryName,
                   const std::string &fileName); // given a json file location,
                                                 // turn file into json object
