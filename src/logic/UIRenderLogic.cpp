...
///    if constexpr (std::is_same_v<std::decay_t<decltype(element_type)>,
///    Panel>) {
///
///      DrawPanel(element);
...
///
///  // call the spacing strategy to adjust the size and position of the
///  children spacing_strategy(element, inner_margin, border_thickness);
///
...
///  // Create a text object for the dropdown list label depending on whether
///  its
///  // expanded
///
...
///      children[i].position.y = ul_element.position.y +
...
///      -
///                           (2 * inner_margin.y);
///      children[i].position.x = ul_element.position.x +
///      parent_border_thickness +
///                               inner_margin.x +
...
///  size_t number_of_children = children.size();
///
///  // need to account for ratios adding up to more than 1.0, or some not
///  having
///  // ratios
///  // default: if sum is greater than 1.0, then we will scale down all the
///  parts
...
///      number_of_children, (1.0f / number_of_children)); // default ratio
///      of 1.0
///  for (size_t i = 0; i < number_of_children; ++i) {
