...
/// @c ChainDescriptorResult(const MachinaFormScaffold&, uint32_t part_id)
...
using ChainDescriptor =
    std::function<ChainDescriptorResult(const MachinaFormScaffold &, uint32_t)>;
...
inline ChainDescriptor lift_to_chain(NodeDescriptor nd) {
  return [nd = std::move(nd)](const MachinaFormScaffold &scaffold,
                              uint32_t id) -> ChainDescriptorResult {
    return ChainDescriptorResult{static_cast<bool>(nd(scaffold, id))};
  };
}
} // namespace steamrot::logic::descriptors
