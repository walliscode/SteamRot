...
void DrawDropDownButtonElement(const UIRenderContext &context,
  ...
  // Check if we need to create or update the cached triangle
  if (!button.render_cache.shape.has_value() ||
      button.render_cache.is_dirty) {
    ...
    button.render_cache.shape = triangle;
    button.render_cache.is_dirty = false;
  }

  // draw the cached triangle on the texture
  context.texture.draw(button.render_cache.shape.value());
}
...
void DrawBorderAndBackground(const UIRenderContext &context,
  ...
  // Check if we need to create or update the cached rectangle
  if (!element.render_cache.rectangle.has_value() ||
      element.render_cache.is_dirty) {
    ...
    // Store in cache and mark as clean
    element.render_cache.rectangle = rectangle;
    element.render_cache.is_dirty = false;
  }

  // Draw the cached rectangle
  context.texture.draw(element.render_cache.rectangle.value());
}
...
void DrawBorderAndBackground(const UIRenderContext &context,
  ...
  // For buttons, we can't fully cache because hover state changes
  // But we can still optimize by checking if we need to update
  
  // Determine the current fill color based on hover state
  sf::Color fill_color = element.is_mouse_over ? style.hover_color
                                                : style.background_color;
  
  // Check if we need to create or update the cached rectangle
  if (!element.render_cache.rectangle.has_value() ||
      element.render_cache.is_dirty ||
      element.render_cache.rectangle.value().getFillColor() != fill_color) {
    // Create the rectangle using the element's position and size
    sf::RectangleShape rectangle(element.size);
    rectangle.setPosition(element.position);
...
void DrawText(const UIRenderContext &context, const std::string &text,
  ...
  // For text, we can't use element.render_cache directly since this is a free function
  // Instead, we create the text object each time but keep the logic for future optimization
  // TODO: Consider passing UIElement to enable caching here as well
...
void UpdateSizeAndPositionOfChildElements(const UIElement &element,
  ...
  if (dynamic_cast<const DropDownContainerElement *>(&element)) {
    ...
    if (!dd_container->child_elements.empty()) {
      auto &child = dd_container->child_elements[0];
      if (child->size != dd_list_size || child->position != dd_list_position) {
        child->size = dd_list_size;
        child->position = dd_list_position;
        child->render_cache.Invalidate();
      }
    }
    ...
    if (dd_container->child_elements.size() > 1) {
      auto &child = dd_container->child_elements[1];
      if (child->size != dd_button_size ||
          child->position != dd_button_position) {
        child->size = dd_button_size;
        child->position = dd_button_position;
        child->render_cache.Invalidate();
      }
    }
  ...
  case Layout::Vertical: {
    ...
    for (size_t i = 0; i < element.child_elements.size(); i++) {
      auto &child = element.child_elements[i];
      sf::Vector2f new_size{available_size.x, child_height};
      sf::Vector2f new_position{
          start_position.x,
          start_position.y +
              i * (child_height + style.panel_style.inner_margin.y)};
      
      if (child->size != new_size || child->position != new_position) {
        child->size = new_size;
  ...
  case Layout::Horizontal: {
    ...
    for (size_t i = 0; i < element.child_elements.size(); i++) {
      auto &child = element.child_elements[i];
      sf::Vector2f new_size{child_width, available_size.y};
      sf::Vector2f new_position{
          start_position.x + i * (child_width + style.panel_style.inner_margin.x),
          start_position.y};
  ...
  case Layout::DropDown: {
    ...
    for (size_t i = 0; i < element.child_elements.size(); i++) {
      auto &child = element.child_elements[i];
      sf::Vector2f new_size{available_size.x, available_size.y};
      sf::Vector2f new_position{start_position.x,
                                start_position.y + i * available_size.y};
