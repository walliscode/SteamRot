...
FlatbuffersUIStyleDataProvider::ProvideUIStyles() {
  ...
  for (const auto style_data : all_ui_styles) {
    ...
    ui_style.panel_style.background_color =
        ToColor(panel_fb->background_color());
    ...
    ui_style.button_style.border_color =
        ToColor(button_style_fb->border_color());
    ui_style.button_style.border_thickness =
        button_style_fb->border_thickness();
    ui_style.button_style.radius_resolution =
        button_style_fb->radius_resolution();
    ui_style.button_style.inner_margin =
        ToVec2f(button_style_fb->inner_margin());
    ui_style.button_style.minimum_size =
        ToVec2f(button_style_fb->minimum_size());
    ui_style.button_style.maximum_size =
        ToVec2f(button_style_fb->maximum_size());
    ...
    ui_style.button_style.font = it->second;
    ...
    ui_style.drop_down_list_style.text_color =
        ToColor(dd_list_fb->text_color());
    ...
    if (get_dd_list_font_result == m_fonts_map.end())
      return std::unexpected(FailInfo{FailMode::FlatbuffersDataNotFound,
                                      "drop_down_list_style.font not found: " +
                                          dd_list_fb->font()->str()});
    ui_style.drop_down_list_style.font = get_dd_list_font_result->second;
    ...
    ui_style.drop_down_item_style.text_color =
        ToColor(dd_item_fb->text_color());
    ...
    if (get_dd_item_font_result == m_fonts_map.end())
      return std::unexpected(FailInfo{FailMode::FlatbuffersDataNotFound,
                                      "drop_down_item_style.font not found: " +
                                          dd_item_fb->font()->str()});
    ui_style.drop_down_item_style.font = get_dd_item_font_result->second;
