...
ConfigureBaseStyle(const StyleDataFbs *style_fb, Style &style,
  ...
  if (style_fb->hover_color()) {
    auto hover_color_result =
        ToColor(style_fb->hover_color(), style_name + ".hover_color");
    if (!hover_color_result.has_value())
      return std::unexpected(hover_color_result.error());
    style.hover_color = hover_color_result.value();
  } else {
    style.hover_color = style.background_color;
  }
