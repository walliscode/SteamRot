...
This document outlines the plan to refactor SteamRot's context and resource
handling system to improve separation of concerns, eliminate builder pattern
complexity, and enable data-driven resource configuration.
...
4. **SceneContext takes three parameters**: SceneResources, GameResources, AND
   EntityManager
...
1. Create Resource structs for objects like RenderWindows, mouse positions,
   RenderTexture
2. Resources should be one-off assets at different levels (GameEngine, Scene,
   etc.)
3. Create Context objects as lightweight reference containers to be passed
   around
4. Rename LogicContext to SceneContext and move it appropriately
5. Resource structs should NOT contain references/pointers to other Resource
   members (no base Resource class needed)
...
8. Create a TestFixture to generate Resource structs and pass Context objects
   for tests
...
2. **No Circular Dependencies**: Resources are standalone, contexts reference
   resources
...
- EntityMemoryPool and Archetypes accessed from EntityManager, not
  SceneResources
...
- Note: Entity pool configuration remains in EntityManager (not in
  SceneResources)
...
- [ ] Update GetSceneContext() to pass SceneResources, GameResources, and
      EntityManager
...
The two-layer architecture (Resources for ownership, Contexts for access) is
cleaner, more maintainable, and aligns with modern C++ best practices.
