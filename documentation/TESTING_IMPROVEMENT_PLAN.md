...
This document outlines a comprehensive plan for improving and implementing a
better test system for the SteamRot game engine. The plan focuses on:

- **Reusability**: Creating test functions that can be reused across different
  test scenarios
- **Extensibility**: Building a test infrastructure that easily accommodates new
  test types
- **Data-Driven Testing**: Enabling easy configuration and feeding of different
  test data
- **Clear Test Classification**: Establishing naming conventions for unit,
  integration, and system tests
- **CI/CD Integration**: Automating testing workflows for continuous quality
  assurance
...
**Definition**: Test a single class or function in isolation with mocked
dependencies.

**Naming Convention**: `{ClassName}.unit.test.cpp` or
`{function_file}.unit.test.cpp`
...
**Current State**: Most existing `.test.cpp` files are actually unit tests but
not explicitly named as such.

#### 2. Integration Tests

**Definition**: Test interactions between multiple components/classes, with some
real dependencies.
...
- `entity_loading.integration.test.cpp` - Tests loading entities from
  FlatBuffers through configurator
- `event_subscription.integration.test.cpp` - Tests event handler + subscriber
  factory interaction
- `ui_collision_render.integration.test.cpp` - Tests UI collision + rendering
  logic together
...
**Definition**: Test complete workflows through the entire system, minimal
mocking.
...
- `player_action.system.test.cpp` - Tests complete player input → action → state
  change
...
- **Example**:
  ...
  ```
- **Benefit**: Can run specific test types: `ctest -L unit` or
  `ctest -L integration`
...
1. Created new directory structure: `tests/unit/`, `tests/system/`,
...
5. Updated README.md with comprehensive testing documentation and CMake preset
   usage
...
**Goal**: Create reusable test components to reduce duplication and improve
maintainability.
...
- **Benefit**: If class B uses class A, any tests for A can be reused via these
  helpers
...
- ⏸️ Updated existing tests to use new infrastructure (deferred to opportunistic
  migration)
...
- ⏸️ At least 5 existing test files refactored to use new infrastructure
  (deferred)
...
3. Created `tests/components/component_test_mixin.h` template for component
   contract tests
...
- Migration to use new infrastructure will happen opportunistically as tests are
  modified
...
**Goal**: Enable tests to run with multiple data configurations without code
duplication.
...
- **Action**: Define extensible system for loading test data
  - each test is a single json file
  - a flatbuffers schema defines the data in the json
  - the flatbuffers table should be extensible so we can add test meta data in
    the future
...
- **Action**: Build utility to load test configurations
  - we want to be cafeful not to dump all jsons/binaries into one directory
  - each division in the test directorys will need its own data directory. With
    each json having some kind of identifier such as .test_data.json, this way
...
- **Action**: Keep the basis of testing reusable
  - try and keep the fundamentals consistent so we can re-use function e.t.c.
  - the game engine will have some key data structures (and we may add more), so
    we need some key testing functions that compare these structures with
    expected vs real
    - EntityMemoryPool comparisons
    - EventBus comparisons
    - others as needed
  - these data structure tests can be wrapped in a function that conditionally
    performs these tests on whether the data is present in the json/binary
  - we will then build upon this with simulation functions, whose purpose is to
    take starting data structures and run through the logic systems. We need to
    be able to define which logic systems are used/combined via some field in
...
- **Files**:
  - Rename `cmake-multi-platform-disabled.txt` →
...
**Incremental Rollout**: Each stage is independently valuable and can be
deployed as completed.
...
2. **CI/CD Resource Usage**: Monitor GitHub Actions minutes, optimize test
   parallelization
...
**Data-Driven**: >20% of tests use data-driven approach ✅ **CI/CD**: All
commits tested automatically within 15 minutes ✅ **Coverage**: >70% overall
code coverage maintained ✅ **Quality**: <5% test failure rate from flakiness ✅
...
- **Test Infrastructure**: Core team maintains TestContext, helpers, base
  classes
...
This comprehensive testing improvement plan provides a structured approach to
enhancing the SteamRot game engine's test infrastructure. By implementing the
plan in stages, the team can realize incremental benefits while minimizing risk
and disruption.
...
The plan is designed to be practical and achievable, with clear deliverables,
success metrics, and timelines for each stage.
