...
before the first tick. `IInputActionConfigProvider` builds the registry from a
data source; `FlatbuffersInputActionConfigProvider` is the production
implementation. The registry is moved into `EventHandler` (and on into
`SFMLEventConverter`) via RAII — no raw pointer to FlatBuffers data is passed
across API boundaries.

```cpp
// In Engine::StartUp() — called once before the first tick.
auto provider = data_access_factory.GetInputActionConfigProvider();  // polymorphic
auto registry = provider.value()->CreateInputActionRegistry();       // built by FlatbuffersInputActionConfigProvider
event_handler.SetInputActionRegistry(std::move(registry.value()));  // RAII move into SFMLEventConverter
```
...
- [ ] Add `case` to `ConfigureInputAction` in `configure_input_action.cpp`
...
- **Unit tests**: `tests/unit/events/sfml_event_convert.test.cpp`,
  ...
  `tests/unit/data_providers/FlatbuffersInputActionConfigProvider.test.cpp`
