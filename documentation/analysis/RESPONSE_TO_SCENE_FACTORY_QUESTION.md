...
### Inside Factory - No Conditionals!
...
FlatbuffersSceneFactory::ConfigureSceneResources(Scene& scene) {
    ...
    // Use the data - Factory doesn't know if it's from default or save!
...
✅ **Solved**: Configurators return `const SceneDataFbs*` directly. No copying.

### "Prevent too much logic for copying data"

✅ **Solved**: Configurators cache the FlatBuffers pointer. Zero copies.

### "Support both ISceneDataProvider and ISaveDataProvider"

✅ **Solved**: Each configurator wraps one provider type. Factory doesn't know the difference.

### "Extensible design"

✅ **Solved**: Want to add network scenes? Create `NetworkSceneConfigurator`. Zero changes to Factory.
...
1. Create `src/scenes/ISceneConfigurator.h`
...
2. Modify `FlatbuffersSceneFactory.cpp` - use configurator
