...
TEST_CASE("EventPacketEqualsMatcher works correctly",
  ...
  expected.event_type = steamrot::EventType::EVENT_USER_INPUT;
  expected.event_id = GenerateTestUUID();
  expected.source_id = GenerateTestUUID();

  steamrot::EventPacket actual{1};

  SECTION("Matcher detects differences in event_type") {
    actual.event_type = steamrot::EventType::EVENT_TEST;
  ...
  SECTION("Matcher detects differences in event_id") {
    actual.event_type = steamrot::EventType::EVENT_USER_INPUT;
