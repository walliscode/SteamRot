...
TEST_CASE("LogicFactory creates the correct Logic instances with a test Scene",
  ...
  steamrot::LogicCollection &logic_map = logic_map_result.value();

  REQUIRE_NOTHROW(logic_map.at(steamrot::LogicType::Collision));
  REQUIRE_NOTHROW(logic_map.at(steamrot::LogicType::Action));
  REQUIRE_NOTHROW(logic_map.at(steamrot::LogicType::Render));

  // evaluate collision logics
  steamrot::LogicVector &collision_logics =
      logic_map.at(steamrot::LogicType::Collision);
  REQUIRE(collision_logics.size() > 0);
  REQUIRE(
      dynamic_cast<steamrot::UICollisionLogic *>(collision_logics[0].get()));

  // evaluate action logics
  steamrot::LogicVector &action_logics =
      logic_map.at(steamrot::LogicType::Action);
  REQUIRE(action_logics.size() > 0);
  REQUIRE(dynamic_cast<steamrot::UIActionLogic *>(action_logics[0].get()));

  // evaluate render logics
  steamrot::LogicVector &render_logics =
      logic_map.at(steamrot::LogicType::Render);
  REQUIRE(render_logics.size() > 0);
