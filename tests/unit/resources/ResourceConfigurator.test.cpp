...
TEST_CASE("ResourceConfigurator with custom scene resource data",
  ...
  auto game_data =
      steamrot::CreateGameResourcesData(fbb2, 800, 600, window_title, 60);
  fbb2.Finish(game_data);

  const steamrot::GameResourcesData *game_resources =
      flatbuffers::GetRoot<steamrot::GameResourcesData>(
  ...
  auto result =
      configurator.ConfigureSceneResources(resources, custom_scene_data);
  REQUIRE(result.has_value());
