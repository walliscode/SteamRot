...
/////////////////////////////////////////////////
// ConfigureInitialAssetConfig tests
/////////////////////////////////////////////////

TEST_CASE("ConfigureInitialAssetConfig fails with null data",
          "[unit][configure_engine_data]") {
  steamrot::AssetConfig config;

  auto result =
      steamrot::data::configure::ConfigureInitialAssetConfig(config, nullptr);
...
TEST_CASE("ConfigureInitialAssetConfig populates from valid data",
          "[unit][configure_engine_data]") {
  // Load real flatbuffers data
  steamrot::FlatbuffersDataLoader loader;
  auto engine_data_result = loader.ProvideEngineDataFbs();
  ...
  const auto *engine_data = engine_data_result.value();
  const auto *asset_config_data = engine_data->initial_asset_config();
  REQUIRE(asset_config_data != nullptr);

  steamrot::AssetConfig config;
  auto result = steamrot::data::configure::ConfigureInitialAssetConfig(
      config, asset_config_data);

  REQUIRE(result.has_value());
  REQUIRE(config.fonts.size() == 2);
  ...
  for (size_t i = 0; i < config.fonts.size(); ++i) {
    REQUIRE(config.fonts[i].name == expected_font_names[i]);
  }

  // verify styles
  REQUIRE(config.ui_styles.size() == 1);
}
