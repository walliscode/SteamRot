...
/////////////////////////////////////////////////
/// @brief Helper function to create and configure an EntityMemoryPool from test
/// data
///
...
CreatePoolFromTestData(const std::string &test_data_name,
...
TEST_CASE("CompareEntityMemoryPools detects equal pools",
  ...
  REQUIRE_NOTHROW(steamrot::tests::CompareEntityMemoryPools(pool1, pool2));
}
...
TEST_CASE("CompareEntityMemoryPools fails for different pool sizes",
  ...
  auto pool1 = CreatePoolFromTestData("pool_comparison_equal", configurator);
  auto pool2 =
      CreatePoolFromTestData("pool_comparison_different_size", configurator);

  // This should throw/fail because the pools have different sizes
  REQUIRE_THROWS(steamrot::tests::CompareEntityMemoryPools(pool1, pool2));
}
...
TEST_CASE("CompareEntityMemoryPools fails for different component values",
  ...
  auto pool1 = CreatePoolFromTestData("pool_comparison_equal", configurator);
  auto pool2 =
      CreatePoolFromTestData("pool_comparison_different_values", configurator);
