...
TEST_CASE("Full configuration flow works correctly",
  ...
  const auto &ui_1 =
      steamrot::entity::memory::GetComponent<steamrot::CUserInterface>(1, emp);
  REQUIRE(ui_1.m_active == true);
  REQUIRE(ui_1.m_name == "simple_ui");
  REQUIRE(ui_1.m_root_element != nullptr);

  // Verify entity 2 (CUserInterface hidden) is configured
  const auto &ui_2 =
      steamrot::entity::memory::GetComponent<steamrot::CUserInterface>(2, emp);
  REQUIRE(ui_2.m_active == true);
  REQUIRE(ui_2.m_name == "hidden_ui");
  REQUIRE(ui_2.m_visible == false);

  // Verify entity 3 (CGrimoireMachina) is configured
  const auto &grimoire_3 =
      steamrot::entity::memory::GetComponent<steamrot::CGrimoireMachina>(3,
                                                                          emp);
  REQUIRE(grimoire_3.m_active == true);

  // Verify entity 4 (CGrimoireMachina with data) is configured
  const auto &grimoire_4 =
      steamrot::entity::memory::GetComponent<steamrot::CGrimoireMachina>(4,
                                                                          emp);
  REQUIRE(grimoire_4.m_active == true);
}
