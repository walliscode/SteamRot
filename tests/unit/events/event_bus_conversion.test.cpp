...
TEST_CASE("ConvertEventBusDataToEventBus handles empty EventBusData",
          "[unit][event_bus_conversion]") {
  // Create empty EventBusData manually (this would normally come from
  // FlatBuffers) Since we can't easily create FlatBuffers data in tests, we'll
  // use a builder
...
TEST_CASE("ConvertEventBusDataToEventBus converts single event",
          "[unit][event_bus_conversion]") {
  flatbuffers::FlatBufferBuilder builder;
  ...
  auto event_packet_offset = steamrot::CreateEventPacketData(
      builder,
      5, // event_lifetime
      steamrot::EventType::EventType_EVENT_TEST, steamrot::EventDataData_NONE);

  std::vector<flatbuffers::Offset<steamrot::EventPacketData>> events;
  events.push_back(event_packet_offset);

  auto event_bus_data_offset =
      steamrot::CreateEventBusData(builder, builder.CreateVector(events));
  builder.Finish(event_bus_data_offset);
  ...
  REQUIRE(result.value()[0].m_event_type ==
          steamrot::EventType::EventType_EVENT_TEST);
  REQUIRE(result.value()[0].event_lifetime == 5);
}

TEST_CASE("ConvertEventBusDataToEventBus converts multiple events",
          "[unit][event_bus_conversion]") {
  flatbuffers::FlatBufferBuilder builder;
  ...
  REQUIRE(result.value()[1].m_event_type ==
          steamrot::EventType::EventType_EVENT_QUIT_GAME);
  REQUIRE(result.value()[1].event_lifetime == 1);
}

TEST_CASE("ConfigureEventHandlerFromEventBusData populates global event bus",
          "[unit][event_bus_conversion]") {
  flatbuffers::FlatBufferBuilder builder;
  ...
  REQUIRE(global_bus[0].m_event_type ==
          steamrot::EventType::EventType_EVENT_TEST);
  REQUIRE(global_bus[0].event_lifetime == 3);
  REQUIRE(global_bus[1].m_event_type ==
          steamrot::EventType::EventType_EVENT_QUIT_GAME);
  REQUIRE(global_bus[1].event_lifetime == 2);
}

TEST_CASE("ConfigureEventHandlerFromEventBusData handles null input",
          "[unit][event_bus_conversion]") {
  steamrot::EventHandler event_handler;
...
TEST_CASE("ConfigureEventHandlerFromEventBusData handles empty EventBusData",
          "[unit][event_bus_conversion]") {
  flatbuffers::FlatBufferBuilder builder;
