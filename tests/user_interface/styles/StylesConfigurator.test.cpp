...
TEST_CASE("StylesConfigurator returns a map of all available styles",
  ...
  steamrot::FlatbuffersDataLoader data_loader{steamrot::EnvironmentType::Test};
  steamrot::StylesConfigurator styles_configurator;
  auto style_names_result = styles_configurator.GetAllStyleNames();
  if (!style_names_result)
    FAIL(style_names_result.error().message);
  // Act
  ...
  // Assert
  REQUIRE(styles_map.size() == style_names_result->size());
  for (const auto &name : *style_names_result) {
