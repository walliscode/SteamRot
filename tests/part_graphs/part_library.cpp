/////////////////////////////////////////////////
/// @file
/// @brief Implementation of the TestPartLibrary struct
/////////////////////////////////////////////////
...
TestPartLibrary TestPartLibrary::Create() {
  ...
  // "fragment_no_socket": green triangle in Front view, no sockets
  {
    ...
    lib.fragments.emplace(FragmentInstanceNames::FragmentNoSocket,
                          std::move(f));
  }
  ...
  // "fragment_one_socket": green origin triangle + one socket at (5, 5)
  {
    ...
    f.positioning_views.insert_or_assign(steamrot::ViewDirection::Front,
                                         MakeGreenOriginTriangle());
    f.sockets = {{5.f, 5.f}};
    lib.fragments.emplace(FragmentInstanceNames::FragmentOneSocket,
                          std::move(f));
  }
  ...
  // 10)
  {
    ...
    lib.fragments.emplace(FragmentInstanceNames::FragmentTwoSockets,
                          std::move(f));
  }
  ...
  // and (20,10)
  {
    ...
    lib.fragments.emplace(FragmentInstanceNames::FragmentThreeSockets,
                          std::move(f));
  }
  ...
  // "joint_no_socket": blue triangle in Front view, no sockets configured
  {
    ...
    lib.joints.emplace(JointInstanceNames::JointNoSocket, std::move(j));
  }
  ...
  // "joint_one_socket": 1 socket at radius 10, full rotation arc
  {
    ...
    lib.joints.emplace(JointInstanceNames::JointOneSocket, std::move(j));
  }
  ...
  // "joint_two_sockets": 2 sockets at radius 15, full rotation arc
  {
    ...
    j.socket_config.rotation_arc_min = 0.f;
    j.socket_config.rotation_arc_max = 360.f;
    lib.joints.emplace(JointInstanceNames::JointTwoSockets, std::move(j));
  }
  ...
  // "joint_three_sockets": 3 sockets at radius 15, full rotation arc
  {
    ...
    j.socket_config.rotation_arc_min = 0.f;
    j.socket_config.rotation_arc_max = 360.f;
    lib.joints.emplace(JointInstanceNames::JointThreeSockets, std::move(j));
  }
  return lib;
}
} // namespace steamrot::tests
